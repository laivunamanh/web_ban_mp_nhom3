...
if (isset($_SESSION['user']['id'])) {
    $total_prod_giohang = total_prod_from_giohang($_SESSION['user']['id']);
}
...
if (isset($_GET['act'])) {
    ...
    switch ($act) {
        ...
        case 'themvaogiohang':
            ...
            if (isset($_SESSION['user']['id'])) {
                ...
                if (isset($_POST['themgiohang'])) {
                    $check = check_giohang($userid, $idsp);
        ...
        case 'capnhatsoluonggiohang':
            if (isset($_SESSION['user']['id'])) {
                $userid = $_SESSION['user']['id'];
                $idsp = $_GET['idsp'];
                $soluong = $_GET['soluong'];
                $onesp = loadone_sanpham_tu_gio_hang($idsp);
                if (intval($onesp["soluong"]) < intval($soluong)) {
                    header("HTTP/1.1 500 OK");
                    die;
                }
                update_giohang($soluong, $idsp);
                die();
            }
            break;
